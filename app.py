...
# Sidebar compacta com estatísticas rápidas
with st.sidebar:
    st.title("💰 Financeiro")
    menu = st.radio(
        "Menu",
    ...
    total_unpaid = run_query("""
        SELECT COALESCE(SUM(amount), 0) as total
        FROM transactions
        WHERE user_id = 1 AND type = 'Despesa' AND status = 'Não Pago'
    """)
    unpaid = float(total_unpaid[0]["total"]) if total_unpaid else 0.0
    st.metric("Total em aberto", format_brl(unpaid), delta=None)
    # Mês atual
...
if menu == "Visão do Mês":
    ...
    # Resumo em cards horizontais (sem scroll)
    resumo = run_query("""
    ...
    if unpaid_count > 0:
        st.warning(f"⚠ {unpaid_count} transação(ões) não paga(s) neste mês", icon="⚠️")
    else:
        st.success("✓ Todas as transações pagas neste mês")

    # Abas internas para organizar conteúdo
    tab1, tab2, tab3 = st.tabs(["📋 Transações", "📊 Gráficos", "⚙️ Ações"])
    ...
    with tab1:
        ...
        if transacoes:
            ...
            # Tabela com altura fixa para não scrollar muito
            st.dataframe(
                ...
                hide_index=True,
                height=400  # altura fixa para evitar scroll infinito
            )
        else:
    ...
    with tab2:
        ...
        with col_g1:
            ...
            fig1.update_layout(title="Resumo do Mês", yaxis_title="Valor (R$)", height=300)
        ...
        with col_g2:
            ...
            if cat_data:
                ...
                fig2 = px.bar(df_cat, x="nome", y="total", title="Despesas por Categoria",
                              color="nome", color_discrete_sequence=px.colors.qualitative.Set2, height=300)
                st.plotly_chart(fig2, use_container_width=True)

    with tab3:
        st.subheader("Gerenciar transação")
        # Seleção por ID com busca
        if transacoes:
            ids = [t["id"] for t in transacoes]
            selected_id = st.selectbox(
                "Selecione a transação pelo ID",
                ids,
                format_func=lambda x: f"ID {x} - {next((t['description'][:30] for t in transacoes if t['id']==x), '')}"
            )
            ...
            with col_a:
                opcao = st.radio("Opção de pagamento", ["Apenas marcar", "Marcar e Abater"], key="pay_option", horizontal=True)
            ...
            st.info("Nenhuma transação para gerenciar.")

    # Modal de edição (usando expander para não ocupar espaço)
    if st.session_state.show_edit_modal and st.session_state.selected_tx_id:
        tx = get_transaction_by_id(st.session_state.selected_tx_id)
        if tx:
            with st.expander("✏️ Editar Transação", expanded=True):
                with st.form("edit_form"):
                    col_e1, col_e2 = st.columns(2)
                    with col_e1:
                        tipo = st.radio("Tipo", ["Despesa", "Receita"], index=0 if tx["type"]=="Despesa" else 1, key="edit_tipo", horizontal=True)
                    ...
                    with col_e2:
                        status = st.radio("Status", ["Não Pago", "Pago"], index=0 if tx["status"]=="Não Pago" else 1, key="edit_status", horizontal=True)
                        paid_date = st.date_input("Data Pagamento", value=tx["paid_date"] or date.today(), key="edit_paid")
                        abatido = st.radio("Abatido do Saldo", ["Não", "Sim"], index=0 if not tx["deducted_from_balance"] else 1, key="edit_abatido", horizontal=True)
                    ...
                    with col_btn1:
                        enviar = st.form_submit_button("💾 Salvar Alterações", use_container_width=True)
                    with col_btn2:
...
# ============================================================
    ...
    if evol:
        ...
        fig.update_layout(barmode="group", title=f"Evolução {ano_atual}", xaxis_title="Mês", yaxis_title="Valor (R$)", height=400)
